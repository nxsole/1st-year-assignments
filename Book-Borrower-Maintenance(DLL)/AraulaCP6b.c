/*	Name: Angel Fredelle Mishka A. Araula
	...
	using doubly linked list. Conversion of singly linked list to doubly linked list.
...
int DataEntry(ndbook *bookHead) {
    ...
    if (confirm == 'Y' || confirm == 'y') {
        ...
        else if (BH->bookBookNum > bTemp->bookBookNum) {
        	bTemp->right = BH;
        	BH->left = bTemp;
        	*bookHead = bTemp;
		}
		 else {
		 	//middle/end of the list 
            p = *bookHead;
            q = p;
            while (p->right != NULL && p->bookBookNum < bTemp->bookBookNum) {
				q = p;
				p = p->right;
            }
            //end of the list
                ...
                if (confirm == 'Y' || confirm == 'y') {
                    if (q == NULL && p->right != NULL) {
                        ...
                        *bookHead = BH;
                    	...
                    	//delete node if there is only one record
...
int BorrDataEntry(ndborr *borrHead) {
    ...
    if (confirm == 'Y' || confirm == 'y') {
        ...
        //beginning of the list
        else if (BH->borrNum > bTemp->borrNum) {
        	bTemp->right = BH;
        	BH->left = bTemp;
        	*borrHead = bTemp;
		}
            ...
            p = *borrHead;
            q = p;
            while (p->right != NULL && p->borrNum < bTemp->borrNum) {
				q = p;
				p = p->right;
            }
            //end of the list
        ...
        printf("Adding borrower to record was cancelled.\n");
    }
    return 0;
}
